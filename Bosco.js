...
break
        case 'exif':
             if (args.length == 0) return reply(`Ex:- ${prefix}exif KRIZ SER`)
             if (!isOwner) return  reply(mess.only.owner)
...
//------------------< Downloader/And Search Media >-------------------
      ...
      case 'more':
       ...
       case 'pinterest':
         case 'pin':
              if (args.length < 1) return reply(`${prefix}KRIZ SER`)
	...
	if (!Number(oi2)) return reply('*The number must be a number!*')
     ...
     case 'tagme':
				...
				if (!isOwner) return 
					...
					if (err) return bosco.sendMessage(from, `root @KRIZ SER:~ ${err}`, text, { quoted: mek })
                 break
              case 'swm':
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							ppp = `${args.join(' ')}`
